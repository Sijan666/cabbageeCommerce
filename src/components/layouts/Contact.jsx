          ...
          {currentProduct ? (
            <div className={`flex flex-col lg:flex-row items-center gap-12 lg:gap-20 transition-all duration-700 ease-in-out ${isFading ? 'opacity-0 translate-y-10' : 'opacity-100 translate-y-0'}`}>
              <div className="w-full lg:w-1/2 flex flex-col items-center lg:items-start text-center lg:text-left z-10">
                ...
                <h3 className="text-[36px] md:text-[50px] lg:text-[60px] text-[#232323] font-extrabold font-int leading-[1.1] mb-4 tracking-tight capitalize hover:text-[#80B500] transition-colors">
                ...
                <div className="flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto">            
                  <div onClick={handleShopNow} className="w-full sm:w-auto block cursor-pointer">
                    <Button btnText={"Shop Now"} />
                  </div>
                  <Link to={`/product/${currentProduct.id}`} className="w-full sm:w-auto block">
                    <button className="cursor-pointer px-8 py-3.5 rounded-full text-[#232323] font-bold font-int text-sm border border-[#232323]/20 hover:bg-[#232323] hover:text-white transition-colors duration-300 w-full">
                      View Details
                    </button>
                  </Link>
                </div>
              ...
              <div className="w-full lg:w-1/2 flex justify-center relative z-10">
                <div className="relative w-75 h-75 md:w-112.5 md:h-112.5 flex justify-center items-center">
                  ...
                  <Link to={`/product/${currentProduct.id}`} className="w-full h-full flex justify-center items-center relative z-20 group">
                    <Images 
                      imgSrc={currentProduct.thumbnail} 
                      className="cursor-pointer w-full h-full object-contain relative z-20 drop-shadow-[0_20px_40px_rgba(0,0,0,0.12)] transition-transform duration-700" 
